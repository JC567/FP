...
HELP_MODE = (
    ...
    '   · 不是调门槛，而是换一套判定标准：质量分≥60(护城河扎实) 且 价格便宜(留安全边际) 且\n'
    '     价值陷阱分≤50(非陷阱)，三者同时成立才"适合"。\n'
    '   · "便宜"判定必须可靠：Gordon合理PE可信(状态VALID)且价≤合理价×0.8 时用 Gordon 安全边际；\n'
    '     否则(A股高股息股普遍为 THIN_SPREAD——股息利差过薄导致 Gordon分母极小、合理PE失真，\n'
    '     如某股算出合理价253元而现价仅86元，明显不可信)退回历史估值分位判定便宜\n'
    '     （PE分位≤30% 或 股息率分位≥70%，即均衡型买点口径）。绝不拿失真 Gordon 充当安全边际。\n'
    '   · 适合时给"分批建仓、长期持有(forever)"建议；不适合时只说明原因、不给出买入建议。\n'
    '   · 与前面三种模式的区别：前三种按"历史估值分位"宽松度买卖；巴菲特模式额外要求质量过硬，\n'
    '     且在 Gordon 不可信时仍以"质量+历史分位便宜+非陷阱"作为价值投资买点。\n'
    '\n'
...
HELP_BACKTEST = (
    ...
    '\n'
    '① 时间范围：默认 2019-01-01 ~ 最近一个交易日，每周再平衡一次。\n'
    ...
    '\n'
    '⑨ 资本预算建仓模式（投资规则改为每年 50 万元）：\n'
    '   下单默认每年初注入 50 万预算，比较以下四种"怎么把这笔钱花出去"的方式（价格用后复权价，\n'
    '   已内含分红次日自动再投；当年未用完的现金自动留存到下一年）：\n'
    '   · 每月定投：每月固定日期投入 50万/12（约 4.17 万），雷打不动，最简单省心。\n'
    '   · 策略买点：出现买入类信号(买入/强烈买入/逢低吸纳)时，把当时全部可用现金一次买完；\n'
    '     若一整年都没出现买点，则现金留存到下一年（预算仍照常注入）。适合"只信信号、平时不动"。\n'
    '   · 智能定投：在每月定投基础上，按估值分位动态调节当月金额——\n'
    '     便宜多买、贵了少买。规则：cheap = 股息率分位 − PE分位（百分点差），\n'
    '     当月倍数 = 限制到[0.5, 2.0]的 (1 + cheap)；即 PE越低/股息率越高(越便宜)买得越多，反之越少。\n'
    '   · 巴菲特模式：仅在"质量分高(护城河扎实，≥60) + 价格便宜(留安全边际) + 非价值陷阱(陷阱分≤50)"\n'
    '     三者同时成立时才建仓；达标后按"每月额度"低估区分批买入（连续低估就逐月买），且永不卖出(持有 forever)。\n'
    '     "便宜"的判定：Gordon合理PE可信(VALID)且价≤合理价×0.8 时用 Gordon；否则退回历史估值分位\n'
    '     （PE分位≤30% 或 股息率分位≥70%）。Gordon 不可信(THIN_SPREAD)时绝不拿失真的合理价充当安全边际。\n'
    '     不满足时现金留存吃国债利息、等下一个机会。这是四者里最贴近"巴菲特式等待+集中+长持"的玩法。\n'
    '   · 闲置现金自动买国债：等待买点/定投间隙的现金，按 10 年期国债收益率每日复利计息（"自动买国债"），\n'
    '     买点/定投触发时连本带息一并换入股，避免资金白白闲置；图中 10Y 国债曲线即其收益参照。\n'
    '   四种模式在"收益率走势图"中以资产(元)曲线对比，并叠加 10 年期国债无风险收益率(%) 作基准。\n'
    '\n'
    '务必冷静看待：\n'
            ...
            for label, key in (('每月定投', 'monthly'), ('策略买点', 'strategy'),
                ...
                if m:
                    A('  · %s：期末资产=%s ｜ 累计投入=%s ｜ 总收益率=%s ｜ 最大回撤=%s'
                      % (label, m.get('期末资产'), m.get('累计投入'),
                         m.get('总收益率'), m.get('最大回撤')))
