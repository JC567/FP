...
HELP_MODE = (
    ...
    '     质量(护城河/稳健)：ROE≥10%(简单口径=归母净利/归母权益) 且 权益比率(资本充足代理)≥6% 且 连续分红≥5年\n'
    '                       且 盈利未显著恶化(连续下滑≤2年、净利最大回撤≥-40%) 且 非价值陷阱(陷阱分≤50)。\n'
    '      便宜(安全边际)：银行业看 PB——有 PB 时 PB≤1.0(破净)=便宜(累积区)；无 PB 时退回历史分位(PE分位≤30% 或 股息率分位≥70%)。\n'
    '      强烈买入区(强买档)：PB≤0.85(深度破净) 或 PE分位≤15% 或 股息率分位≥85%——安全边际更深。\n'
    '   · 以上门槛同时成立才"适合"；适合时给"分批建仓、长期持有(forever)"建议，分两档：\n'
...
HELP_BACKTEST = (
    ...
    '     买点分两档（均每月最多一次、不一次性）：强烈买入区(深度破净PB≤0.85 或 PE分位≤15% 或 股息率分位≥85%)\n'
    '     每月额度 = 常规档×2；累积区(刚破净/边际低估)每月额度 = 常规档。低估越深建仓越快、占比越大。\n'
    '     非银行业标的：回测直接不建仓，并提示"暂未支持该行业"。这是最贴近"巴菲特式等待+集中+长持+能力圈"的玩法。\n'
